...
export class SignupPage implements OnInit {
  ...
  async signUp() {
    ...
    const user = await this.authservice.register(this.credentials.value as User).then(async (res)=>{
    ...
    }).catch((err)=>{this.showAlert('Registration Failed', 'Please try again')});
  ...
  async getUserInfo(uid:string){
    const loading = await this.loadingControler.create();
    await loading.present();
    const user = await this.authservice.getDocument(`users/${uid}`).then((res)=>{
      console.log(res);
